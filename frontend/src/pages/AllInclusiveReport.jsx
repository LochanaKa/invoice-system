...
export default function AllInclusiveReport() {
  const now   = new Date();
  ...
  async function fetchReport() {
    ...
    try {
      ...
      setError("Could not load All-Inclusive report. Is the FastAPI server running?");
  ...
  async function handleDownloadPDF() {
    ...
    try {
      ...
      a.href     = url;
      a.download = `ALL_INC_Summary_${year}_${String(month).padStart(2,"0")}.pdf`;
      a.click();
    ...
    <div className="space-y-5 max-w-6xl">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Business watch summary for ALL_INC invoices · monthly totals and export
          </p>
        </div>
      ...
      {loading && !report && (
        <div className="flex items-center justify-center py-16 gap-2 text-gray-400">
          ...
          <span className="text-sm">Loading All-Inclusive data…</span>
        </div>
      )}
      ...
      {report && (
        <>
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
            {[
              { label: "Invoices",     value: report.invoice_count,      color: "#1F3C8A" },
              { label: "Total Taxable", value: fmtLKR(report.total_taxable), color: "#1F3C8A" },
              { label: "Total SSCL",    value: fmtLKR(report.total_sscl),    color: "#7c3aed" },
              { label: "Total VAT",     value: fmtLKR(report.total_vat),     color: "#7c3aed" },
          ...
          {report.invoice_count === 0 ? (
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-5 text-center">
              ...
              <div className="text-gray-500 text-sm font-medium">
                No All-Inclusive invoices for {MONTHS[month-1]} {year}
              </div>
              <div className="text-gray-400 text-xs mt-1">
                Try a different month or create a new ALL_INC invoice
              </div>
            </div>
          ) : (
            <div className="bg-white rounded-2xl border overflow-hidden"
              ...
              <div className="px-5 py-3 border-b"
                ...
                <h2 className="text-xs font-bold uppercase tracking-wide"
                    style={{ color: "#1F3C8A" }}>
                  All-Inclusive Invoices — {MONTHS[month-1]} {year}
                </h2>
                  ...
                  <thead>
                    <tr style={{ background: "#f7f9ff", borderBottom: "2px solid #eef1fb" }}>
                      {["#", "Invoice No.", "Date", "Customer", "Customer TIN",
                        "Taxable Value", "SSCL", "VAT (18%)", "Grand Total"].map((h) => (
                        <th key={h}
